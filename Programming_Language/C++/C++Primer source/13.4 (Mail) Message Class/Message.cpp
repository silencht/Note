...
Message::Message(Message &&m) : contents(std::move(m.contents)) //个人猜测，std::move的使用，使得初始化contents的方式从copy变为调用string类型的移动构造函数？
{
...
Message& Message::operator=(const Message &rhs)
{
	...
	remove_from_Folders();	// 更新已有Folder：使自身所在的 folder中删除自身
...
{
	if (this != &rhs) {		// 直接检查自赋值情况，参数&&是右值引用，if的条件&是取rhs的地址
		remove_from_Folders();
